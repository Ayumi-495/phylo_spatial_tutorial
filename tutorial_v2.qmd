...
We use a sample dataset on Sylviidae (passerines), which is a CSV file with species data and a Nexus file containing a phylogenetic tree.
The dataset only includes 10 species for didactic purposes, while the tree contains 294 species. So, we need to prune the tree to match the dataset…
```{r}
