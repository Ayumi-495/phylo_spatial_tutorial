...
# (brms sigma) + one squared-exponential GP over recorded coordinate locations.
...
form <- brms::bf(
  d_Hedges | se(sqrt(var_Hedges), sigma = TRUE) ~
    1 + gp(x_km, y_km, cov = "exp_quad", scale = FALSE)
)

# The exp_quad GP correlation is exp(-d^2 / (2*lscale^2)). Its e-folding
# distance is sqrt(2) * lscale, so the prior below has median 50 km on the
# common e-folding-distance scale used for all three packages.
priors <- c(
  brms::prior(normal(0, 1), class = "Intercept"),
  brms::prior(student_t(3, 0, 1), class = "sigma"),
  brms::prior(student_t(3, 0, 1), class = "sdgp", coef = "gpx_kmy_km"),
  brms::prior(lognormal(log(50 / sqrt(2)), 1), class = "lscale", coef = "gpx_kmy_km")
)
...
writeLines(c(
  ...
  "formula: d_Hedges | se(sqrt(var_Hedges), sigma=TRUE) ~ 1 + gp(x_km, y_km, cov=exp_quad, scale=FALSE).",
  "sampling model: Normal(mu, sqrt(se_i^2 + sigma^2)); sigma is iid effect-size SD.",
  "spatial model: exp_quad GP covariance sdgp^2 * exp(-distance^2/(2*lscale^2)) over unique recorded coordinate pairs.",
  "Common e-folding distance: sqrt(2) * lscale kilometres.",
...
summary_rows <- rbind(
  ...
  pick_summary("^lscale_gpx_kmy_km$|^lscale\\(gpx_kmy_km\\)$", "lscale_km")
)
...
summary_rows$e_folding_range_km <- NA_real_
summary_rows$e_folding_range_lb_km <- NA_real_
summary_rows$e_folding_range_ub_km <- NA_real_
is_lscale <- summary_rows$parameter == "lscale_km"
summary_rows$e_folding_range_km[is_lscale] <- sqrt(2) * summary_rows$estimate[is_lscale]
summary_rows$e_folding_range_lb_km[is_lscale] <- sqrt(2) * summary_rows$ci_lb[is_lscale]
summary_rows$e_folding_range_ub_km[is_lscale] <- sqrt(2) * summary_rows$ci_ub[is_lscale]
...
writeLines(c(
  ...
  "The exp_quad GP e-folding distance is sqrt(2) * lscale in km because scale=FALSE and x_km/y_km were supplied."
