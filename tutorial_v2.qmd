...
The following legacy package-specific examples have not been rerun under the
verified study/location structure and are intentionally omitted from the
rendered tutorial pending a separate package-comparison audit.
...
Spatial meta-analysis models residual similarity as a function of separation among recorded coordinate locations. The required input is a distance matrix whose row and column order matches the levels of the location factor used in the model. For global data, calculate ellipsoidal great-circle distances in kilometres from longitude and latitude rather than projecting all observations to EPSG:3857.
...
The `metafor` comparison estimates a spatial variance ($\tau^2$) and a scale parameter ($\rho$). For an exponential correlation, $\rho$ controls the rate at which correlation declines with distance. Neither parameter should be interpreted solely from its point estimate: profile likelihoods assess whether the data distinguish the parameter from plausible alternatives.

Spatial variance ($\tau^2$) describes the amount of heterogeneity assigned to the spatial component. The scale parameter ($\rho$) describes how the exponential correlation changes with great-circle distance. Both parameters can be weakly identified, so their profile likelihoods, rather than their point estimates alone, should guide interpretation.

The sampling-error structure is held fixed across the three models through `V = var_Hedges`, which is diagonal under the independent-sampling-error assumption used here.

The pooled mean remains negative and similar across the three covariance specifications. This robustness does not extend to the allocation of heterogeneity or to inference about spatial structure. The spatial-only model is a restricted model, and the combined model does not establish a well-defined spatial correlation range once study-level heterogeneity is included.
