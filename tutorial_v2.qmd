...
![Moura BM meta-analysis fitted with `metafor`. The pooled effect and each variance component are shown with their 95% confidence intervals; variance rows use component-specific horizontal scales.](figs/tutorial/metafor_moura2021.png){fig-alt="Moura metafor BM results showing the pooled Fisher's Z mean and four variance components with confidence intervals." width="80%"}
...
![Moura BM meta-analysis fitted with `glmmTMB`. The pooled effect has its reported 95% Wald confidence interval; variance rows are fitted point estimates on component-specific horizontal scales.](figs/tutorial/tmb_eg1_1.png){fig-alt="Moura glmmTMB BM results showing the pooled Fisher's Z mean and four fitted variance components." width="80%"}
...
# `fit_phylo_eg2_metafor_mr` is the model fitted in the metafor tab above.
phylo_eg2_brms_mr <- readRDS(here("Rdata", "tutorial_v2", "phylo_eg2_brms_mr.rds"))
...
![Lim BM meta-regression fitted with `metafor`. Fixed effects have 95% confidence intervals; variance rows are fitted point estimates on component-specific horizontal scales.](figs/tutorial/metafor_lim.png){fig-alt="Lim metafor BM meta-regression results showing fixed effects and three fitted variance components." width="80%"}
...
tmb_p2
```
![Lim BM meta-regression fitted with `glmmTMB`. Fixed effects have Wald 95% confidence intervals; variance rows are fitted point estimates on component-specific horizontal scales.](figs/tutorial/tmb_eg2_mr.png){fig-alt="Lim glmmTMB BM meta-regression results showing fixed effects and three fitted variance components." width="80%"}
:::
...
The precomputed `metafor` result below is generated from the validated spatial-only fit. The pooled-effect interval is a 95% t-based confidence interval. Saved profile-likelihood 95% confidence intervals are shown for the spatial variance and range; the iid effect-size variance is shown as its fitted point estimate because a separate profile interval was not retained for that component. The three panels have different units and horizontal scales, so their widths should not be compared directly.

![Spain regional subset: spatial-only `metafor` result.](figs/tutorial/spain_metafor_spatial_only.png){fig-alt="Metafor results for the Spain regional spatial-only model: pooled effect with its t confidence interval, iid and spatial variance estimates, and exponential range with its profile confidence interval." width="85%"}
