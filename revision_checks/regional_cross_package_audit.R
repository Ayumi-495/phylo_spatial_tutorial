...
write_metadata <- function(extra = character()) {
  writeLines(c(
    ...
    "Target model: intercept + iid effect-size heterogeneity + exponential spatial covariance.",
    extra
...
if (stage == "metafor") {
  ...
  fit_capture <- capture_conditions({
    metafor::rma.mv(
      ...
      struct = "SPEXP", dist = list(site_id = p$D_proj),
...
if (stage == "glmmTMB") {
  ...
  # For exp, theta is log(SD), log(range inverse), i.e. correlation
  # exp(-exp(-theta[2]) * distance); therefore rho = exp(theta[2]) km.
  ...
  write_metadata(c(
    ...
    "glmmTMB spatial parameterisation: exp(-exp(-theta[2]) * distance_km); common e-folding rho = exp(theta[2]) km.",
