...
Unfortunately, we cannot fit the OU model using `brms` or `glmmTMB` yet…
...
Most spatial meta-analytic models assume that residual similarity between effect sizes decays smoothly with geographic distance. The distance matrix $\mathbf{D}$ provides the raw spatial information needed to define this decay.

Depending on the packages and kernels (functions) used, the distance matrix $\mathbf{D}$ is incorporated in different ways:

-   metafor uses $\mathbf{D}$ implicitly when you specify spatial correlation structures such as "SPLIN (linear)""SPEXP" (exponential) or "SPGAU" (Gaussian) (more detailed info: <https://wviechtb.github.io/metafor/reference/rma.mv.html>).

-   brms uses the coordinates directly but constructs an equivalent distance-based kernel internally.

-   glmmTMB uses spatial indexing factors that are also derived from these distances.

Mathematically, all of these approaches rely on the same underlying distances.
