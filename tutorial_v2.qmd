...
If the phylogenetic relationships among the species you want to analyse are already known and you have access to a corresponding tree (e.g., from a published source — for birds, you can usually obtain one from [BirdTree.org](BirdTree.org)), you can easily generate a phylogenetic correlation matrix using simple codes.
```{r}
...
##   if the file contains multiple trees, it is better to start by a single tree.
##   using ~50 trees is generally considered sufficient for phylogenetic meta-analysis to check the necessity of phylogeny.
